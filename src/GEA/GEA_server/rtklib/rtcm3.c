...
static int decode_msm_head(rtcm_t *rtcm, int sys, int *sync, int *iod,
                           msm_h_t *h, int *hsize, int *staidp)
{
    msm_h_t h0={0};
    double tow,tod;
    char *msg,tstr[64];
    ...
    *h=h0;
    if (i+157<=rtcm->len*8) {
        staid     =getbitu(rtcm->buff,i,12);       i+=12;
        trace(3,"sat: %d referenceStationId: %d \n",j ,staid);
        *staidp = staid;
        ...
        if (sys==SYS_GLO) {
            ...
            tow   =getbitu(rtcm->buff,i,30)*0.001; i+=30;
            trace(3,"sat: %d gnssEpochTime: %d \n",j ,tow);
            adjweek(rtcm,tow);
        }
        *sync     =getbitu(rtcm->buff,i, 1);       i+= 1;
        trace(3,"sat: %d multipleMessage: %d \n",j ,*sync);
        *iod      =getbitu(rtcm->buff,i, 3);       i+= 3;
        trace(3,"sat: %d issueOfDataStation: %d \n",j ,*iod);
        h->time_s =getbitu(rtcm->buff,i, 7);       i+= 7;
        trace(3,"sat: %d time_s: %d \n",j ,h->time_s);
        h->clk_str=getbitu(rtcm->buff,i, 2);       i+= 2;
        trace(3,"sat: %d clockSteeringIndicator: %d \n",j ,h->clk_str);
        h->clk_ext=getbitu(rtcm->buff,i, 2);       i+= 2;
        trace(3,"sat: %d externalClockIndicator: %d \n",j ,h->clk_ext);
        h->smooth =getbitu(rtcm->buff,i, 1);       i+= 1;
        trace(3,"sat: %d divergenceFreeSmoothingIndicator: %d \n",j ,h->smooth);
        h->tint_s =getbitu(rtcm->buff,i, 3);       i+= 3;
        trace(3,"sat: %d smoothingInterval: %d \n",j ,h->tint_s);
        for (j=1;j<=64;j++) {
...
{
    msm_h_t h={0};
    int i,sync,iod,staid;
    if (decode_msm_head(rtcm,sys,&sync,&iod,&h,&i,&staid)<0) return -1;
    rtcm->obsflag=!sync;
    return sync?0:1;
}
...
static int decode_msm4(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,type,sync,iod,ncell,rng,rng_m,prv,cpv,lock[64],half[64],staid;
    
    type=getbitu(rtcm->buff,24,12);
    
    /* decode msm header */
    if ((ncell=decode_msm_head(rtcm,sys,&sync,&iod,&h,&i,&staid))<0) return -1;
...
static int decode_msm5(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,type,sync,iod,ncell,rng,rng_m,rate,prv,cpv,rrv,lock[64],staid;
    int ex[64],half[64];
    ...
    trace(3,"sat: %d QueEstacionEs: %d \n",j ,staid);
    if(staid == 1234)
    {
        /* decode satellite data */
        for (j=0;j<h.nsat;j++) { /* range */
            rng  =getbitu(rtcm->buff,i, 8); i+= 8;
            trace(3,"sat: %d roughRangeIntegerMilliseconds: %d \n",j ,rng);
            if (rng!=255) r[j]=rng*RANGE_MS;

            ex[j]=getbitu(rtcm->buff,i, 4); i+= 4;
            trace(3,"sat: %d extendedInformation: %d \n",j ,ex[j]);

            rng_m=getbitu(rtcm->buff,i,10); i+=10;
            trace(3,"sat: %d roughRangeModulo1Millisecond: %d \n",j ,rng_m);
            if (r[j]!=0.0) r[j]+=rng_m*P2_10*RANGE_MS;

            rate =getbits(rtcm->buff,i,14); i+=14;
            trace(3,"sat: %d roughPhaserangeRateMetersPerSecond: %d \n",j ,rate);
            if (rate!=-8192) rr[j]=rate*1.0;
        }
        /* decode signal data */
        for (j=0;j<ncell;j++) { /* pseudorange */
            prv=getbits(rtcm->buff,i,15); i+=15;
            trace(3,"sat: %d finePseudorange: %d \n",j ,prv);
            if (prv!=-16384) pr[j]=prv*P2_24*RANGE_MS;

            cpv=getbits(rtcm->buff,i,22); i+=22;
            trace(3,"sat: %d finePhaserange: %d \n",j ,cpv);
            if (cpv!=-2097152) cp[j]=cpv*P2_29*RANGE_MS;

            lock[j]=getbitu(rtcm->buff,i,4); i+=4;
            trace(3,"sat: %d phaserangeLockTimeIndicator: %d \n",j ,lock[j]);

            half[j]=getbitu(rtcm->buff,i,1); i+=1;
            trace(3,"sat: %d halfCycleAmbiguityIndicator: %d \n",j ,half[j]);

            cnr[j]=getbitu(rtcm->buff,i,6)*1.0; i+=6;
            trace(3,"sat: %d cnr: %d \n",j ,cnr[j]);

            rrv=getbits(rtcm->buff,i,15); i+=15;
            trace(3,"sat: %d finePhaserangeRate: %d \n",j ,rrv);
